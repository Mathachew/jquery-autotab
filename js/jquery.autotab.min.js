...
a.loaded=!0;c(b).data("autotab-defaults",a);return c(b).on("autotab-next",function(b,a){a.target.length&&!a.iOS&&(a.target.focus().select(),f=new Date)}).on("autotab-previous",function(b,a){if(a.previous.length){a.previous.focus();if(this.setSelectionRange){var c=a.previous.val().length;a.previous[0].setSelectionRange(c,c)}else a.previous.val(a.previous.val());f=null}}).on("keydown",function(a){var b=c(this).data("autotab-defaults"),g=a.which||a.charCode;if(8==g&&0==this.value.length&&b.previous.length)c(this).trigger("autotab-previous",
b);else if(9==g&&null!=f)if(a.shiftKey)f=null;else{if(800>(new Date).getTime()-f.getTime())return f=null,!1}else f=null}).on("keypress",function(a){var b=c(this).data("autotab-defaults"),g=a.which||a.keyCode,d=m(String.fromCharCode(g),b),l=document.selection&&document.selection.createRange?!0:0<a.charCode;if(a.ctrlKey||a.altKey)return!0;if(l&&(null==d||""==d))return a.ctrlKey;if(l&&this.value.length<=this.maxLength){var e;if("number"==typeof this.selectionStart&&"number"==typeof this.selectionEnd){e=
this.selectionStart;a=this.selectionEnd;if(0==e&&a==this.value.length)this.value=d;else{if(this.value.length==this.maxLength)return c(this).trigger("autotab-next",b),!1;this.value=this.value.slice(0,e)+d+this.value.slice(a)}this.selectionStart=this.selectionEnd=e+1}else if(document.selection&&document.selection.createRange){a=document.selection.createRange();l=this.createTextRange();e=this.createTextRange();var f=a.getBookmark();l.moveToBookmark(f);e.setEndPoint("EndToStart",l);e=e.text.length;a=
e+a.text.length;if(0==e&&a==this.value.length)this.value=d;else{if(this.value.length==this.maxLength)return c(this).trigger("autotab-next",b),!1;this.value=this.value.slice(0,e)+d+this.value.slice(a)}e++;l=this.createTextRange();l.collapse(!0);l.move("character",e-(this.value.slice(0,e).split("\r\n").length-1));l.select()}}if(-1=="8,9,16,17,18,19,20,27,33,34,35,36,37,38,39,40,45,46,144,145".indexOf(g)&&void 0!=g)return this.value=m(this.value,b),this.value.length==b.maxlength&&c(this).trigger("autotab-next",
b),!1}).on("paste",function(a){var b=function(a,d,f){if(a){var e=c(a).data("autotab-defaults"),h=a.maxLength;c(a).data("autotab-defaults",e);a.maxLength=2147483647;setTimeout(function(){null==d&&(d=a.value);var k;k=null!=f?m(d.substr(d.indexOf(f)+f.length),e).substr(0,h):m(d,e).substr(0,h);a.maxLength=h;k?(a.value=k,k.length==h&&(c(a).trigger("autotab-next",e),e.iOS||b(e.target[0],d,k))):a.value=""},1)}};b(this,null,null)})};c.fn.autotab_magic=function(b){c(this).autotab()};c.fn.autotab_filter=function(b){var d=
{};"string"==typeof b||"function"==typeof b?d.format=b:c.extend(d,b);c(this).autotab("filter",d)}})(jQuery);
